    ...
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <MetricCard
          ...
          description={stats ? `${stats.totals.totalPremiumUsers} premium (${premiumPercentage}%)` : '...'}
          link={{href: '/admin/users', text: 'View All Users'}}
        />
        <MetricCard
          title="Active Users (7d)"
          value={stats?.totals.activeLast7Days ?? '0'}
          icon={<Activity className="h-4 w-4 text-muted-foreground" />}
          description={stats ? `${stats.breakdowns.byGender['male'] || 0} M / ${stats.breakdowns.byGender['female'] || 0} F` : '...'}
        />
        <MetricCard
          title="Pending Verifications"
          value={stats?.totals.pendingVerifications ?? '0'}
          icon={<UserCheck className="h-4 w-4 text-muted-foreground" />}
          link={{href: '/admin/moderation/verify', text: 'Review now'}}
        />
        <MetricCard
          title="AI Matchmaking"
          value={<Wand2 className="h-6 w-6" />}
          icon={<HeartHandshake className="h-4 w-4 text-muted-foreground" />}
          description="Generate compatible pairs."
          link={{href: '/admin/matchmaking', text: 'Run AI Suggestions'}}
        />
      </div>
